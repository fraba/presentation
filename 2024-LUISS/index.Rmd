...
From the social media data, we created two network types: 
...
1\. **Link reciprocity ratio**, or the proportion of mutual connections in a directed network
...
2\. **Standard deviation of the in-degree distribution**, which tells how much a single user can have incoming links well above the average for that network. It captures the capacity (for some but not for all) to be seen by a large portion of the network. 
...
3\. **Link transitivity ratio**, which quantifies how many of the potential triangles in a network are actual triangles.
...
4\. **Mean node distance**, or the average number of steps to traverse the network. 
...
- Finally, each textual posting (posts or replies) in the data was assigned a **proximity weight** to all topics: this allows to understand if a particular posting discusses a topic but also the relative distance among users in terms of what they discuss.
...
- **Variables** available to the voters **through the interface of the voting platform** (e.g. screen position, gender) are **consistently more important**, followed by different platform-specific measures of centrality, with Meetup.com and Facebook.com appearing as more central.
...
- Centrality of a user **in the cross-platform network** shows a **benefit for more active candidates**.

---

### What predicts ranking in the M5S 2012 Online Primary elections?

- As a block, **associations between candidates and topics did not improve predictions**.
...
2. We contribute to the debate on intra-party digitalization by providing evidence of **decentralization dynamics** in a political organization heavily dependent on digital platforms: this **supports centrifugal theories**.
...
```{r, results = "asis", echo=FALSE}
PrintBibliography(myBib, start = 13, end = 17)
```
...
```{r, results = "asis", echo=FALSE}
PrintBibliography(myBib, start = 18)
```
