...
format:
  revealjs:
    ...
    footer: "[Slides: https://francescobailo.net/65IFXRKL/](https://francescobailo.net/65IFXRKL/)"
...
DATA_DIR <- "/Users/francesco/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/research/2025-info-disorder-us-election-in-2020/LLM-coding/run-2/results/"
...
INFON_RESULTS_DEM <- "/Users/francesco/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/research/2025-info-disorder-us-election-in-2020/LLM-coding/run-2/06_classify_infons/results/posts_sampled_democrat_results_3models_20251211_034121.csv"
INFON_RESULTS_REP <- "/Users/francesco/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/research/2025-info-disorder-us-election-in-2020/LLM-coding/run-2/06_classify_infons/results/posts_sampled_republican_results_3models_20251211_034121.csv"
INFON_RESULTS_GEN <- "/Users/francesco/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/research/2025-info-disorder-us-election-in-2020/LLM-coding/run-2/06_classify_infons/results/posts_sampled_general_results_3models_20251211_034121.csv"
```
...
# =============================================================================
# Function to create a representative signed network with independence
# =============================================================================
...
create_network <- function(n_nodes, 
  ...
  edge_type <- matrix(NA, n_total, n_total)
  rownames(edge_type) <- colnames(edge_type) <- node_ids
    ...
    for (i in 1:(n_nodes-1)) {
      for (j in (i+1):n_nodes) {
        ...
        if (rand < p_agreement) {
          adj_matrix[i, j] <- 1
          adj_matrix[j, i] <- 1
          edge_type[i, j] <- "agreement"
          edge_type[j, i] <- "agreement"
        } else if (rand < (p_agreement + p_disagreement)) {
          adj_matrix[i, j] <- 1
          adj_matrix[j, i] <- 1
          edge_type[i, j] <- "disagreement"
          edge_type[j, i] <- "disagreement"
        } else if (rand < (p_agreement + p_disagreement + p_independence)) {
          adj_matrix[i, j] <- 1
          adj_matrix[j, i] <- 1
          edge_type[i, j] <- "independence"
          edge_type[j, i] <- "independence"
        }
        # Otherwise: no edge
      }
    }
  ...
  if (include_supra) {
    ...
    for (i in 1:n_nodes) {
      rand <- runif(1)
      if (rand < 0.30) {
        adj_matrix[i, supra_idx] <- 1
        adj_matrix[supra_idx, i] <- 1
        edge_type[i, supra_idx] <- "agreement"
        edge_type[supra_idx, i] <- "agreement"
      } else if (rand < 0.60) {
        adj_matrix[i, supra_idx] <- 1
        adj_matrix[supra_idx, i] <- 1
        edge_type[i, supra_idx] <- "disagreement"
        edge_type[supra_idx, i] <- "disagreement"
      } else if (rand < 0.85) {
        adj_matrix[i, supra_idx] <- 1
        adj_matrix[supra_idx, i] <- 1
        edge_type[i, supra_idx] <- "independence"
        edge_type[supra_idx, i] <- "independence"
      }
      # Otherwise: no edge
    }
  }
  ...
  # Set edge attributes from edge_type matrix
  edge_list <- as_edgelist(g)
  edge_types <- sapply(1:nrow(edge_list), function(i) {
    from_name <- edge_list[i, 1]
    to_name <- edge_list[i, 2]
    edge_type[from_name, to_name]
  })
  E(g)$relationship <- edge_types
...
plot_network_ggraph <- function(g) {
  ...
  tg <- as_tbl_graph(g) %>%
    ...
    mutate(relationship = factor(relationship, 
                                 levels = c("agreement", "disagreement", "independence"),
                                 labels = c("Agreement", "Disagreement", "Independence")))
  
  # Create plot
  p <- ggraph(tg, layout = "fr") +
    # Edges
    geom_edge_link(aes(color = relationship, linetype = relationship), 
                   edge_width = 1.5, alpha = 0.8) +
    scale_edge_color_manual(values = c("Agreement" = "#2E8B57", 
                                       "Disagreement" = "#DC143C",
                                       "Independence" = "#888888"),
                           name = "Relationship") +
    scale_edge_linetype_manual(values = c("Agreement" = "solid", 
                                          "Disagreement" = "solid",
                                          "Independence" = "dashed"),
                              name = "Relationship") +
    # Nodes
    geom_node_point(aes(size = type, fill = type), 
                    shape = 21, color = "gray30", stroke = 1.5) +
    scale_size_manual(values = c("Post" = 12, "Supra-infon" = 20),
                     name = "Node Type") +
    scale_fill_manual(values = c("Post" = "#87CEEB", "Supra-infon" = "#FFD700"),
                     name = "Node Type") +
    # Labels
...
coef_table <- read_csv("assets/sharecount_coefficients_table.csv", show_col_types = FALSE)

coef_table %>%
  select(Sample = sample, Outcome = outcome, `Coefficient (log-odds)` = coefficient, 
         `95% CI` = ci, `p-value` = p_value, Sig = sig) %>%
  kable(align = c("l", "l", "r", "c", "r", "c"))
```
...
![](assets/sharecount_coefficients_faceted.svg){width="100%"}
...
::: aside
Model: Logistic regression predicting stance from log(shareCount), controlling for temporal effects. Coefficients in log-odds.
:::
